...
class News extends CActiveRecord {
    ...
    public function getNewsList($args, $cookieLangType) {
        if (strtolower($cookieLangType) == Yii::app()->params['ENGLISH']) {
            $query = Yii::app()->db->createCommand()->select('n.id as news_id,n.news_header_E as news_header,n.news_desc_E as news_desc,n.news_long_desc_E as news_long_desc,n.news_image as image,n.news_year as date')->from('tbl_news n')
                    ->where('status=' . "'1'");
            ...
            if (!empty($args['year'])) {
                $query->andWhere('year(news_year)=' . $args['year']);
            }
        } else if (strtolower($cookieLangType) == Yii::app()->params['CHINESE']) {
            $query = Yii::app()->db->createCommand()->select('n.id as news_id,n.news_header_C as news_header,n.news_desc_C as news_desc,n.news_long_desc_C as news_long_desc,n.news_image as image,n.news_year as date')->from('tbl_news n')
                    ->where('status=' . "'1'");
            if ((!Yii::app()->user->isGuest && Yii::app()->user->aUserData['roleType'] == 'A') || (isset($args['preview']) && !empty($args['preview']))) {
    ...
    public function getNewsDetail($args, $cookieLangType) {
        if (strtolower($cookieLangType) == Yii::app()->params['ENGLISH']) {
            $query = Yii::app()->db->createCommand()->select('n.id as news_id,n.news_header_E as news_header,n.news_desc_E as news_desc,n.news_long_desc_E as news_long_desc,n.news_image as image,n.news_year as date')->from('tbl_news n')
                    ->where('n.id=' . $args['news_id'])
                    ->andWhere('status=' . "'1'");
