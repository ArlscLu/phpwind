...
class EButtonColumnWithClearFilters extends CButtonColumn {
    ...
    public function init()
    {
        ...
        //define defaults
        $_optionsDefault=array('class'=>'clear','id'=>'cbcwr_clear','style'=>'text-align:left;display:block;');
        ...
        if (!empty($imageUrl)) {
            ...
            $_imageUrl = Yii::app()->request->baseUrl.'/images/refresh.png';
        }
        ...
        //Vineet : Atered by vineet to remove the custome buttom configuration
        $arrButton = array();
        $arrButton = $this->buttons;
        $this->buttons = array(
        'clear' => $this->_clearButton,
        );

        $this->buttons = array_merge($arrButton,$this->buttons);
