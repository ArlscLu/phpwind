 ...
 <div class="col-xs-12 col-sm-6 ">
   ...
   <div class="col-sm-12 " ><div class="col-xs-12 lang-cat1">CN</div></div>
	  ...
	  <div class="col-xs-12 col-sm-12 ">  <?php echo $form->labelEx($model, 'address_chi'); ?></div>
       ...
       <div class="col-xs-12 col-sm-12 ">  <textarea name="address_chi" id="address_chi" class="editor"><?php echo $model->address_chi;  ?>
        </textarea>
	...
	<div class="col-xs-12 col-sm-6   "> <?php echo $form->labelEx($model, 'city_eng'); ?></div>
	 ...
	 <div class="col-xs-12 col-sm-6 ">  <?php echo $form->labelEx($model, 'shop_name_eng'); ?></div>
      ...
      <div class="col-xs-12 col-sm-12 ">   <?php echo $form->labelEx($model, 'address_eng'); ?></div>
       ...
       <div class="col-xs-12 col-sm-12 ">  <textarea name="address_eng" id="address_eng" class="editor"><?php echo $model->address_eng;  ?>
        </textarea>
...
<script type="text/javascript">
    ...
    function validate (){
        ...
        if(jQuery(contentTxtE).length == 0){
            jQuery('#errormsgcontent_E').html('<label  style = "margin-left:10px;color:#f00; " class="error" >Please enter store name (English).</label>');
            error = true;
        }  
        else {  
        ...
        if(jQuery(contentTxtC).length == 0){
            jQuery('#errormsgcontent_C').html('<label  style = "margin-left:10px;color:#f00; " class="error" >Please enter store name(Chinese)</label>');
            error = true;
        }  
        else {  
        ...
        if (error == false) {
            ...
            $.ajax({
                ...
                type: 'POST',
                url: '<?php echo Yii::app()->createAbsoluteUrl("store/update?id=".$model->id); ?>',
                data: formD,
